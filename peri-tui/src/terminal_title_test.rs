...
fn test_terminal_title_format_working_unnamed() {
    ...
    assert_eq!(
        item.format_title(),
        "⠋ peri",
        "未命名时 Working 为 ⠋ project"
    );
}

#[test]
fn test_terminal_title_format_working_named() {
    let item = TerminalTitleItem::new(
...
fn test_sanitize_title_collapses_whitespace_runs() {
    ...
    let cleaned = sanitize_title(raw);
    assert_eq!(
        cleaned, "Project | Working | Thread",
        "未将空白符折叠为单空格并去除首尾空格"
    );
}
...
fn test_extract_thread_title_english_simple() {
    ...
    assert_eq!(
        title.as_deref(),
        Some("fix the issue where terminal"),
        "英文前缀过滤与词数截断不符合预期"
    );
}
