...
pub(crate) struct BareAgentConfig {
    ...
    /// TUI 事件 channel sender，用于创建子 Agent 专用 handler（绕过共享 event handler 锁竞争）
    ...
    > = child_event_tx.map(|tx_child| {
        ...
        > = Arc::new(move |agent_id: String| {
            ...
            Arc::new(FnEventHandler(move |event: ExecutorEvent| {
                ...
                // 映射为 TUI AgentEvent 并直接发送到 channel
