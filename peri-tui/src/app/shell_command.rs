...
impl App {
    pub fn run_shell_command(&mut self, command: String) {
        ...
        let task_anchor_message_id = anchor_message_id.clone();
        let handle = tokio::spawn(async move {
            ...
            let record = ShellCommandRecord {
                ...
                started_at,
                completed_at,
                anchor_message_id: task_anchor_message_id,
            };
    ...
    pub(crate) fn cancel_shell_command(&mut self) -> bool {
        let record = {
            let shell = &mut self.session_mgr.current_mut().shell_command;
            let Some(record_id) = shell.running_record_id.clone() else {
                return false;
            };
            if let Some(abort_handle) = shell.abort_handle.take() {
                abort_handle.abort();
            }
            shell.stdin_tx = None;
            ...
            ShellCommandRecord {
                id: record_id,
                thread_id: shell.thread_id.clone().unwrap_or_default(),
                ...
                anchor_message_id: shell.anchor_message_id.clone(),
            }
        };
