...
fn measure_syntaxset_load_cost() {
    ...
    println!("\n=== 总计 ===");
    println!(
        "SyntaxSet + ThemeSet: RSS = {} KB (+{} KB = {:.2} MB)",
        after_ts,
        total,
        total as f64 / 1024.0
    );
...
fn measure_full_highlight_first_call_cost() {
    ...
    println!(
        ...
        after_second,
        delta_second,
        delta_second as f64 / 1024.0
    );
    println!(
        "    返回行数: {}",
        result2.as_ref().map(|v| v.len()).unwrap_or(0)
    );
    ...
    println!("    RSS = {} KB (+{} KB)", after_python, delta_python);
    ...
    println!("\n=== 结论 ===");
    println!(
        "syntect SyntaxSet + ThemeSet 首次加载 = {:.2} MB",
        delta_first as f64 / 1024.0
    );
