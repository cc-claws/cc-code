...
**文件**: `perihelion-widgets/src/list.rs`
...
2. `new()` 初始化新字段：`mouse_pos: None, on_select: None`
...
**验证**: `cargo build -p perihelion-widgets` 编译通过
...
4. `StatefulWidget::render` 修改——三态样式选择：
...
**文件**: `perihelion-widgets/src/list.rs`（内联 `#[cfg(test)] mod tests`）
...
**验证**: `cargo test -p perihelion-widgets --lib -- list`

---

## Step 4: ListOverlay 浮动容器

**文件**: `perihelion-widgets/src/list_overlay.rs`（**新增**）
...
**文件**: `perihelion-widgets/src/list_overlay.rs`（内联 `#[cfg(test)] mod tests`）
...
**验证**: `cargo test -p perihelion-widgets --lib -- list_overlay`

---

## Step 6: lib.rs 导出

**文件**: `perihelion-widgets/src/lib.rs`
...
**验证**: `cargo build -p perihelion-widgets` 编译通过；`cargo test -p perihelion-widgets` 全量通过

---

## Step 7: PanelList<T> 统一状态管理器

**文件**: `rust-agent-tui/src/app/panel_list.rs`（**新增**）
...
**验证**: `cargo build -p rust-agent-tui` 编译通过；`cargo test -p rust-agent-tui --lib -- panel_list`

---

## Step 8: PanelComponent trait 增加 handle_mouse

**文件**: `rust-agent-tui/src/app/panel_component.rs`
...
2. 在 `handle_scroll` 方法之后新增 `handle_mouse` 默认实现：
...
**验证**: `cargo build -p rust-agent-tui` 编译通过（所有面板通过默认实现兼容）

---

## Step 9: PanelManager 增加 dispatch_mouse + dispatch_scroll 统一

**文件**: `rust-agent-tui/src/app/panel_manager.rs`
...
**文件**: `rust-agent-tui/src/event.rs`
...
2. **统一滚轮分发**：将 McpPanel、PluginPanel 的滚轮处理从 ad-hoc 改为走 `dispatch_scroll`：
...
3. **新增鼠标点击分发**：在 `Down(MouseButton::Left)` 分支中，面板区域优先拦截：
...
**验证**: `cargo build -p rust-agent-tui`；手动测试 McpPanel/PluginPanel 滚轮行为不变

---

## Step 11: AgentPanel 迁移到 PanelList\<AgentItem\>

**文件**: `rust-agent-tui/src/app/agent_panel.rs`
...
2. 移除 `cursor`、`scroll_offset` 字段和 `move_cursor()` 方法。新增委托方法：
...
**文件**: `rust-agent-tui/src/app/model_panel.rs`
...
2. `new()` 中 `list.set_items(vec![AliasTab::Opus, AliasTab::Sonnet, AliasTab::Haiku, AliasTab::Effort])`。

3. `move_cursor` 委托到 `self.list.move_cursor(delta)`——**行为变更**: 从循环变为 clamp。4 项固定列表，效果几乎等价。

4. `handle_key` 中 `self.cursor` 改为 `self.list.cursor()`。
...
**文件**: `rust-agent-tui/src/app/memory_panel.rs`
...
2. 移除 `cursor`、`scroll_offset` 字段和 `move_cursor_up/down` 方法。
...
**文件**: `rust-agent-tui/src/app/hooks_panel.rs`
...
2. 移除 `cursor`、`scroll_offset` 字段和 `move_cursor()` 方法。
...
6. 新增 `handle_mouse`（信息型面板，点击仅移动 cursor）：
...
**文件**: `rust-agent-tui/src/app/cron_state.rs`
...
2. 移除 `tasks`、`cursor`、`scroll_offset` 字段和 `move_cursor()` 方法。
...
6. 新增 `handle_mouse`（信息型，点击仅移动 cursor）：
...
**文件**: `rust-agent-tui/src/app/mcp_panel.rs`
...
2. `new()` 中初始化 `server_list: PanelList::new()`，然后 `server_list.set_items(servers.clone())`。

3. 移除 `cursor` 和 `scroll_offset` 字段。新增委托：
...
4. `do_move_up/do_move_down` 变更——按视图分派到 `server_list.move_cursor()` 或 detail cursor。

5. `handle_scroll` 变更——按视图分派到 `server_list.handle_scroll()` 或 `detail_scroll_offset`。
...
7. servers 列表变更后同步更新 `server_list`：`self.server_list.set_items(self.servers.clone())`。
...
**文件**: `rust-agent-tui/src/app/plugin_panel.rs`
...
2. `new()` 中初始化各 `PanelList`。

3. 委托方法——按视图返回对应的 `list.cursor()` 和 `list.scroll_offset()`。

4. `handle_scroll` 变更——按视图分派到对应的 `PanelList::handle_scroll`。

5. 新增 `handle_mouse`——按视图分派到对应的 `PanelList::handle_mouse_click`。

6. 视图切换时同步 items：`self.installed_list.set_items(self.entries.clone())` 等。
...
**文件**: `rust-agent-tui/src/app/config_panel.rs`
...
2. `from_config()` 中 `browse_list.set_items(vec![Autocompact, CompactThreshold, Language, Persona, Tone, Proactiveness])`。

3. `handle_key` Browse 模式变更：
...
4. 新增 `handle_scroll`（Browse 模式）和 `handle_mouse`（Browse 模式点击进入编辑，Edit 模式点击 toggle 切换）。
...
**文件**: `rust-agent-tui/src/app/login_panel.rs`
...
2. `from_config()` 中 `browse_list.set_items(providers.clone())`。

3. `move_cursor` 委托到 `browse_list.move_cursor(delta)`。

**行为变更**: Browse 导航从循环变为 clamp。

4. 新增 `handle_scroll`（Browse 模式）和 `handle_mouse`（Browse 模式点击进入编辑）。
...
**文件**: `rust-agent-tui/src/app/panel_ops.rs`
...
5. 移除 `ensure_cursor_visible()` 函数——已由 `PanelList::ensure_visible` 覆盖

**验证**: `cargo build -p rust-agent-tui` 编译通过；确认无其他调用方

---

## Step 21: 移除 App 上的 ad-hoc 面板滚动方法

**文件**: `rust-agent-tui/src/app/mod.rs`（或对应 impl App 块）
...
**验证**: `cargo build -p rust-agent-tui` 编译通过；`cargo test -p rust-agent-tui` 全量通过
...
- `perihelion-widgets/src/list.rs` — ListState 鼠标字段/方法、SelectableList 三态渲染、hover_style
- `perihelion-widgets/src/list_overlay.rs` — **新增**: ListOverlay/ListOverlayState/Anchor/OverlayPosition
- `perihelion-widgets/src/lib.rs` — 模块导出
...
- `rust-agent-tui/src/event.rs` — 鼠标事件统一路由、提取 mouse_in_rect、消除 ad-hoc 滚轮代码
- `rust-agent-tui/src/app/agent_panel.rs` — 标准面板迁移的参考实现
- `rust-agent-tui/src/app/panel_ops.rs` — 移除遗留辅助函数
- `rust-agent-tui/src/app/mod.rs` — 移除 ensure_cursor_visible
