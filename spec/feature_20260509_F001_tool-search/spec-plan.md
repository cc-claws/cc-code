...
本次改动涉及 2 个 crate：`rust-create-agent`（executor 工具收集过滤）和 `rust-agent-middlewares`（tool_search 模块：core_tools / search index / 两个元工具 / middleware 集成）。Task 1-4 为纯新增模块（零修改现有文件），Task 5 修改 executor.rs 的工具收集逻辑，Task 6 完成 middleware 集成和 TUI agent 组装。依赖链：Task 1 → Task 2 → Task 3/4（并行）→ Task 5 → Task 6。
...
- [x] 验证测试框架可用
  - `cargo test -p rust-create-agent --lib --no-run 2>&1 | tail -3`
  - 预期: 编译成功，可运行测试
- [x] 验证 rust-agent-middlewares 构建可用
...
- [x] 测试框架可运行
  - `cargo test -p rust-create-agent --lib -- --list 2>&1 | head -3`
  - 预期: 列出测试名称，无配置错误
...
**涉及文件:**
- 新建: `rust-agent-middlewares/src/tool_search/mod.rs`
- 新建: `rust-agent-middlewares/src/tool_search/core_tools.rs`
- 修改: `rust-agent-middlewares/src/lib.rs` (添加 `pub mod tool_search;` 声明和 re-export)

**执行步骤:**
- [x] 创建 tool_search 模块目录结构和 mod.rs 入口文件
  - 位置: `rust-agent-middlewares/src/tool_search/mod.rs` (新建)
...
- [x] 实现 CORE_TOOLS 白名单和 META_TOOLS 集合
  - 位置: `rust-agent-middlewares/src/tool_search/core_tools.rs` (新建)
...
- [x] 实现 is_deferred_tool() 判定函数
  - 位置: `rust-agent-middlewares/src/tool_search/core_tools.rs` (在 CORE_TOOLS 和 META_TOOLS 定义之后)
...
- [x] 在 lib.rs 中声明 tool_search 模块并导出公共接口
  - 位置: `rust-agent-middlewares/src/lib.rs` (在 `pub mod skills;` 之后，`pub mod tools;` 之前)
...
- [x] 为 core_tools.rs 编写单元测试
  - 测试文件: `rust-agent-middlewares/src/tool_search/core_tools.rs` (在文件末尾添加 `#[cfg(test)] mod tests` 模块)
  - 测试场景:
  ...
  - 运行命令: `cargo test -p rust-agent-middlewares --lib tool_search::core_tools::tests`
  - 预期: 所有测试通过，覆盖 Core/Meta/Deferred/未知四种情况

**检查步骤:**
- [x] 验证模块编译通过
  - `cargo build -p rust-agent-middlewares 2>&1 | grep -E "(Compiling|Finished|error)" | head -10`
  - 预期: 输出包含 "Compiling rust-agent-middlewares" 和 "Finished"，无 error

- [x] 验证 CORE_TOOLS 包含全部 12 个核心工具
  - `grep -A 20 'pub static CORE_TOOLS' rust-agent-middlewares/src/tool_search/core_tools.rs | grep -o '"[^"]*"' | wc -l`
  - 预期: 输出为 12（12 个核心工具名）

- [x] 验证 is_deferred_tool 函数可被外部调用
  - `grep -r 'pub use tool_search::is_deferred_tool' rust-agent-middlewares/src/`
  - 预期: 在 lib.rs 中找到一行 re-export 声明

- [x] 验证单元测试覆盖所有分支
  - `cargo test -p rust-agent-middlewares --lib tool_search::core_tools::tests -- --nocapture 2>&1 | grep -E "(test core_tool|test meta_tool|test deferred_tool|test unknown_tool|passed)"`
...
**涉及文件:**
- 新建: `rust-agent-middlewares/src/tool_search/search_tool.rs`

**执行步骤:**
- [x] 创建 SearchExtraTools 结构体实现 BaseTool trait
  - 位置: 新建 `rust-agent-middlewares/src/tool_search/search_tool.rs`
  - 添加依赖引入: `use std::sync::Arc; use async_trait::async_trait; use rust_create_agent::tools::BaseTool; use serde_json::Value; use crate::tool_search::tool_index::ToolSearchIndex;`
...
- [x] 在 mod.rs 中导出 SearchExtraTools
  - 位置: `rust-agent-middlewares/src/tool_search/mod.rs`
  - 添加: `pub mod search_tool; pub use search_tool::SearchExtraTools;`
  - 原因: 使 ToolSearchMiddleware 可以使用此工具

- [x] 为 SearchExtraTools 编写单元测试
  - 测试文件: `rust-agent-middlewares/src/tool_search/search_tool.rs` 的 `#[cfg(test)] mod tests` 块
  - 测试场景:
  ...
  - 运行命令: `cargo test -p rust-agent-middlewares --lib tool_search::search_tool`
  - 预期: 所有测试通过

**检查步骤:**
- [x] 验证文件编译通过
  - `cargo build -p rust-agent-middlewares 2>&1 | grep -E "(error|warning.*search_tool)"`
  - 预期: 无错误或警告

- [x] 验证工具名称和描述正确
  - `cargo test -p rust-agent-middlewares --lib tool_search::search_tool::tests::test_tool_name_is_SearchExtraTools`
  - 预期: 测试通过，工具名称为 "SearchExtraTools"

- [x] 验证 parameters schema 格式正确
  - `cargo test -p rust-agent-middlewares --lib tool_search::search_tool::tests::test_parameters_schema`
  - 预期: 测试通过，schema 包含 query 属性且 required

- [x] 验证搜索功能正常工作
  - `cargo test -p rust-agent-middlewares --lib tool_search::search_tool::tests::test_invoke_search_returns_results`
...
**涉及文件:**
- 新建: `rust-agent-middlewares/src/tool_search/mod.rs`
- 新建: `rust-agent-middlewares/src/tool_search/keyword_search.rs`
- 新建: `rust-agent-middlewares/src/tool_search/tool_index.rs`
...
- [x] 更新 `rust-agent-middlewares/src/lib.rs` 导出 tool_search 模块
  - 位置: 文件末尾添加 `pub mod tool_search;`
  - 原因: 使 tool_search 模块对 crate 外部可见（Task 3/4 需要引用）

- [x] 为 ToolSearchIndex 编写单元测试
  - 测试文件: `rust-agent-middlewares/src/tool_search/tool_index_tests.rs`（或新建 `tests/tool_index_test.rs`）
  - 测试场景:
  ...
  - 运行命令: `cargo test -p rust-agent-middlewares --lib tool_search`
  - 预期: 所有测试通过

**检查步骤:**
- [x] 验证编译通过
  - `cargo build -p rust-agent-middlewares 2>&1 | grep -E "(error|warning:.*tool_search)"`
  - 预期: 无错误，无 tool_search 相关警告
- [x] 验证模块导出正确
  - `grep -n "pub mod tool_search" rust-agent-middlewares/src/lib.rs`
  - 预期: 找到一行声明
- [x] 验证公共 API 可用
  - `grep -E "pub (fn|struct|enum)" rust-agent-middlewares/src/tool_search/*.rs | head -20`
...
- [x] 在 ReActAgent 结构体新增 tool_filter 字段
  - 位置: `rust-create-agent/src/agent/executor.rs` 中 ReActAgent 结构体定义（约 L30-50 区域，在现有字段之后）
  - 添加字段: `tool_filter: Option<fn(&str) -> bool>,`
  - 字段类型: `Option<fn(&str) -> bool>`，None 表示不过滤（向后兼容），Some(filter) 表示按 filter 函数过滤工具
  - 原因: 过滤函数由上层（TUI agent 组装时）注入，避免 rust-create-agent 依赖 rust-agent-middlewares

- [x] 实现 with_deferred_callback() builder 方法
  - 位置: `rust-create-agent/src/agent/executor.rs` 中 ReActAgent 的 builder 方法区域（约 L80-120，与现有 with_xxx 方法并列）
...
- [x] 在工具收集过滤逻辑后调用 deferred_callback
  - 位置: `rust-create-agent/src/agent/executor.rs` 的 execute() 方法中，在 tool_refs 构建之后、`self.chain.run_before_agent(state)` 之前
  ...
  - 关键伪代码:
    ...
    ```
  - 注意: `Arc::from(&dyn BaseTool)` 需要 BaseTool 实现对象安全，需使用 `BoxToolWrapper` 包装后转 Arc，或使用 `ArcToolWrapper`。参考 `rust-agent-middlewares/src/tools/mod.rs:25` 的 `BoxToolWrapper` 模式
  - 原因: 将 deferred tools 传递给外部 ToolSearchIndex，使 SearchExtraTools 能搜索到它们

- [x] 修改工具收集逻辑应用过滤
  - 位置: `rust-create-agent/src/agent/executor.rs` 的 execute() 方法中工具收集部分（约 L168，将 `let tool_refs: Vec<&dyn BaseTool> = all_tools.values().copied().collect();` 替换为过滤逻辑）
  - 替换为以下逻辑:
    ```rust
...
- [x] 为工具过滤逻辑编写单元测试
  - 测试文件: `rust-create-agent/src/agent/executor.rs` 的 `#[cfg(test)] mod tests` 模块（在现有测试之后追加）
  - 测试场景:
  ...
  - 运行命令: `cargo test -p rust-create-agent --lib agent::executor::tests::test_tool_filter`
  - 预期: 所有测试通过，验证过滤逻辑正确且向后兼容

**检查步骤:**
- [x] 验证编译通过
  - `cargo build -p rust-create-agent 2>&1 | grep -E "(Compiling rust-create-agent|Finished|error)"`
  - 预期: 输出包含 "Compiling rust-create-agent" 和 "Finished"，无 error

- [x] 验证 tool_filter 字段存在
  - `grep -n "tool_filter: Option<fn(&str) -> bool>" rust-create-agent/src/agent/executor.rs`
  - 预期: 找到一行字段声明

- [x] 验证 with_tool_filter 方法存在
  - `grep -n "pub fn with_tool_filter" rust-create-agent/src/agent/executor.rs`
  - 预期: 找到一行方法声明

- [x] 验证过滤逻辑正确应用
  - `grep -A 5 "let tool_refs: Vec<&dyn BaseTool>" rust-create-agent/src/agent/executor.rs | grep -E "(filter|collect)"`
  - 预期: 找到包含 filter 逻辑的代码段

- [x] 验证单元测试通过
  - `cargo test -p rust-create-agent --lib agent::executor::tests::test_tool_filter -- --nocapture 2>&1 | grep -E "(test |passed|FAILED)"`
...
**涉及文件:**
- 新建: `rust-agent-middlewares/src/tool_search/execute_tool.rs`

**执行步骤:**
- [x] 创建 ExecuteExtraTool 结构体实现 BaseTool trait
  - 位置: 新建 `rust-agent-middlewares/src/tool_search/execute_tool.rs`
  - 添加依赖引入: `use std::sync::Arc; use async_trait::async_trait; use rust_create_agent::tools::BaseTool; use serde_json::Value; use crate::tool_search::tool_index::ToolSearchIndex;`
...
- [x] 为 ExecuteExtraTool 编写单元测试
  - 测试文件: `rust-agent-middlewares/src/tool_search/execute_tool.rs` 的 `#[cfg(test)] mod tests` 块
  - 测试场景:
  ...
  - 运行命令: `cargo test -p rust-agent-middlewares --lib tool_search::execute_tool`
  - 预期: 所有测试通过

**检查步骤:**
- [x] 验证文件编译通过
  - `cargo build -p rust-agent-middlewares 2>&1 | grep -E "(error|warning.*execute_tool)"`
  - 预期: 无错误或警告

- [x] 验证工具名称和描述正确
  - `cargo test -p rust-agent-middlewares --lib tool_search::execute_tool::tests::test_tool_name_is_ExecuteExtraTool`
  - 预期: 测试通过，工具名称为 "ExecuteExtraTool"

- [x] 验证 parameters schema 格式正确
  - `cargo test -p rust-agent-middlewares --lib tool_search::execute_tool::tests::test_parameters_schema`
  - 预期: 测试通过，schema 包含 tool_name 和 params 两个属性且 required

- [x] 验证代理执行逻辑正常工作
  - `cargo test -p rust-agent-middlewares --lib tool_search::execute_tool::tests::test_invoke_executes_deferred_tool`
  - 预期: 测试通过，成功查找并执行目标工具

- [x] 验证错误处理逻辑正确
  - `cargo test -p rust-agent-middlewares --lib tool_search::execute_tool::tests::test_tool_not_found_returns_error`
...
**涉及文件:**
- 新建: `rust-agent-middlewares/src/tool_search/middleware.rs`
- 修改: `rust-agent-middlewares/src/tool_search/mod.rs`（导出 ToolSearchMiddleware）
- 修改: `rust-agent-middlewares/src/lib.rs`（导出 ToolSearchMiddleware）
- 修改: `rust-agent-tui/src/app/agent.rs`（组装 ToolSearchMiddleware + 注入 tool_filter）
- 修改: `rust-agent-middlewares/src/hitl/mod.rs`（HITL 权限透传）

**执行步骤:**
- [x] 创建 ToolSearchMiddleware 实现 Middleware trait
  - 位置: 新建 `rust-agent-middlewares/src/tool_search/middleware.rs`
  - 添加依赖引入: `use std::sync::Arc; use async_trait::async_trait; use rust_create_agent::middleware::r#trait::Middleware; use rust_create_agent::agent::state::State; use rust_create_agent::error::AgentResult; use rust_create_agent::tools::BaseTool; use crate::tool_search::tool_index::ToolSearchIndex; use crate::tool_search::search_tool::SearchExtraTools; use crate::tool_search::execute_tool::ExecuteExtraTool;`
...
- [x] 在 lib.rs 中导出 ToolSearchMiddleware
  - 位置: `rust-agent-middlewares/src/lib.rs`（在现有 tool_search 导出区域）
  - 添加: `pub use tool_search::ToolSearchMiddleware;`
  - 原因: 使 rust-agent-tui 可以导入 ToolSearchMiddleware

- [x] 在 agent.rs 中创建 ToolSearchIndex 并组装 ToolSearchMiddleware
  - 位置: `rust-agent-tui/src/app/agent.rs` 的 `run_universal_agent()` 函数中（约 L269-339 区域）
  - 在 L269 `ReActAgent::new(model)` 之前添加: `let tool_search_index = Arc::new(rust_agent_middlewares::tool_search::ToolSearchIndex::new());`
  ...
  - 在 L335 MCP 中间件注册之后添加: `let executor = executor.add_middleware(Box::new(rust_agent_middlewares::tool_search::ToolSearchMiddleware::new(Arc::clone(&tool_search_index))));`
...
- [x] 在 agent.rs 中注入 tool_filter 到 ReActAgent
  - 位置: `rust-agent-tui/src/app/agent.rs` 的 `run_universal_agent()` 函数中（在 L269 `ReActAgent::new(model)` 链式调用中添加）
  - 在 L270 `.max_iterations(500)` 之后添加: `.with_tool_filter(rust_agent_middlewares::tool_search::is_deferred_tool)`
...
- [x] 为 ToolSearchMiddleware 编写单元测试
  - 测试文件: `rust-agent-middlewares/src/tool_search/middleware.rs` 的 `#[cfg(test)] mod tests` 块
  - 测试场景:
  ...
  - 运行命令: `cargo test -p rust-agent-middlewares --lib tool_search::middleware`
  - 预期: 所有测试通过

- [x] 为 HITL 权限透传编写单元测试
  - 测试文件: `rust-agent-middlewares/src/hitl/mod.rs` 的 `#[cfg(test)] mod tests` 块
  - 测试场景:
  ...
  - 运行命令: `cargo test -p rust-agent-middlewares --lib hitl::tests::test_execute_extra_tool_permission`
  - 预期: 所有测试通过

- [x] 为 agent.rs 组装编写集成测试
  - 测试文件: `rust-agent-tui/src/app/agent.rs` 的 `#[cfg(test)] mod tests` 块
  - 测试场景:
  ...
  - 运行命令: `cargo test -p rust-agent-tui --lib app::agent::tests::test_tool_search_integration`
  - 预期: 所有测试通过

**检查步骤:**
- [x] 验证 ToolSearchMiddleware 编译通过
  - `cargo build -p rust-agent-middlewares 2>&1 | grep -E "(Compiling rust-agent-middlewares|Finished|error)"`
  - 预期: 输出包含 "Compiling rust-agent-middlewares" 和 "Finished"，无 error

- [x] 验证 ToolSearchMiddleware 导出正确
  - `grep -n "pub use tool_search::ToolSearchMiddleware" rust-agent-middlewares/src/lib.rs`
  - 预期: 找到一行 re-export 声明

- [x] 验证 agent.rs 中间件注册顺序
  - `grep -A 3 "McpMiddleware::new" rust-agent-tui/src/app/agent.rs | grep -c "ToolSearchMiddleware"`
  - 预期: 找到 ToolSearchMiddleware 在 MCP 之后注册

- [x] 验证 agent.rs tool_filter 注入
  - `grep -n "with_tool_filter" rust-agent-tui/src/app/agent.rs`
  - 预期: 找到一行 `with_tool_filter(rust_agent_middlewares::tool_search::is_deferred_tool)`

- [x] 验证 HITL effective_tool_name 函数存在
  - `grep -n "pub fn effective_tool_name" rust-agent-middlewares/src/hitl/mod.rs`
  - 预期: 找到一行函数声明

- [x] 验证 HITL before_tool 使用 effective_tool_name
  - `grep -A 2 "if !(self.requires_approval)" rust-agent-middlewares/src/hitl/mod.rs | grep "effective_tool_name"`
  - 预期: 找到 effective_tool_name 调用

- [x] 验证 ToolSearchMiddleware 单元测试通过
  - `cargo test -p rust-agent-middlewares --lib tool_search::middleware -- --nocapture 2>&1 | grep -E "(test |passed|FAILED)"`
  - 预期: 输出包含所有测试名称，全部 passed，无 FAILED

- [x] 验证 HITL 权限透传单元测试通过
  - `cargo test -p rust-agent-middlewares --lib hitl::tests::test_execute_extra_tool_permission -- --nocapture 2>&1 | grep -E "(test |passed|FAILED)"`
  - 预期: 输出包含所有测试名称，全部 passed，无 FAILED

- [x] 验证 agent.rs 集成测试通过
  - `cargo test -p rust-agent-tui --lib app::agent::tests::test_tool_search_integration -- --nocapture 2>&1 | grep -E "(test |passed|FAILED)"`
  - 预期: 输出包含所有测试名称，全部 passed，无 FAILED

- [x] 验证 TUI 构建成功
  - `cargo build -p rust-agent-tui 2>&1 | grep -E "(Compiling rust-agent-tui|Finished|error)"`
  - 预期: 输出包含 "Compiling rust-agent-tui" 和 "Finished"，无 error
...
**前置条件:**
- 启动命令: `cargo build -p rust-agent-tui`
- 所有 Task 0-6 已完成
...
2. 验证 CORE_TOOLS 白名单完整（12 个核心工具）
   - `cargo test -p rust-agent-middlewares --lib tool_search::core_tools::tests -- --nocapture 2>&1 | grep -E "(passed|FAILED)"`
   - 预期: 所有 Core/Meta/Deferred 判定测试通过
   - 失败排查: 检查 Task 1 `core_tools.rs`

3. 验证搜索引擎正确工作
   - `cargo test -p rust-agent-middlewares --lib tool_search::tool_index -- --nocapture 2>&1 | grep -E "(passed|FAILED)"`
   - 预期: 索引构建、搜索排序、工具查找测试通过
   - 失败排查: 检查 Task 2 `tool_index.rs` 和 `keyword_search.rs`

4. 验证元工具正确实现
   - `cargo test -p rust-agent-middlewares --lib tool_search::search_tool -- --nocapture 2>&1 | grep -E "(passed|FAILED)"`
   - `cargo test -p rust-agent-middlewares --lib tool_search::execute_tool -- --nocapture 2>&1 | grep -E "(passed|FAILED)"`
   - 预期: SearchExtraTools 和 ExecuteExtraTool 测试通过
   - 失败排查: 检查 Task 3 `search_tool.rs` 和 Task 4 `execute_tool.rs`

5. 验证 executor 工具过滤逻辑
   - `cargo test -p rust-create-agent --lib agent::executor::tests::test_tool_filter -- --nocapture 2>&1 | grep -E "(passed|FAILED)"`
   - 预期: 过滤逻辑测试通过，向后兼容
   - 失败排查: 检查 Task 5 `executor.rs`

6. 验证中间件集成和 HITL 权限透传
   - `cargo test -p rust-agent-middlewares --lib tool_search::middleware -- --nocapture 2>&1 | grep -E "(passed|FAILED)"`
   - `cargo test -p rust-agent-middlewares --lib hitl::tests::test_execute_extra_tool_permission -- --nocapture 2>&1 | grep -E "(passed|FAILED)"`
